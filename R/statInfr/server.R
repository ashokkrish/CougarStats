statInfrServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    onemeanraw_iv$add_rule("sample1", sv_regex(
                                        "^( )*(-)?([0-9]+(\\.[0-9]+)?)(,( )*(-)?[0-9]+(\\.[0-9]+)?)+([ \r\n])*$",
...
### leaving it commented out for now
  ...
  coord_cartesian(clip = "off")
    ...
    OneMeanUploadStatus <- reactive({
      if (is.null(fileInputs$oneMeanStatus) || fileInputs$oneMeanStatus == "reset") {
        return(NULL)
    ...
    observeEvent(input$dataAvailability, {
      ...
      } else if (is.null(OneMeanUploadStatus())) {
        hideTab("onePopMeanTabset", "Uploaded Data", session = getDefaultReactiveDomain())
      }
    })
    ...
    output$onePopMeanUploadTable <- renderDT({
      validate(
        need(!is.null(input$oneMeanUserData), "Please upload a file."),
    ...
    ## is pressed; the uploaded data is already shown immediately (to be
    ...
    observeEvent(input$resetInference, {
      ## NOTE: it should only be hid because the uploaded data is discarded;
      ## however, because the session hasn't ended and we don't want to give the
      ## user the impression that they can "delete" the data on demand (a false
      ## security), not allowing this is better and simplifies many things.
      ## NOTE: instructions to users: Just select another data source, or upload
      ## a different file. When you disconnect your uploaded data, the most
      ## recent as well as any prior data, are deleted when your session ends.
      ## To request your data be deleted immediately, disconnect from the
      ## website (close your browser tab or navigate away).
      hideTab("onePopMeanTabset", "Uploaded Data")

      ## FIXME: modifying UI that is not visible anyways is silly and expensive.
      hide("anovaUploadInputs")
      hide("kwUploadInputs")
