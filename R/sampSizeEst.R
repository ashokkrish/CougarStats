...
sampSizeEstServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    ## -------- Reactives ------------------------------------------------------
    #  ========================================================================= #
    criticalValueMean <- reactive({
      
      if(input$confLevelnMean == "90%") {
        critVal <- 1.645
      } else if(input$confLevelnMean == "95%") {
        critVal <- 1.96
      } else if(input$confLevelnMean == "99%") {
        critVal <- 2.576
      }
      return(critVal)
    })
    
    criticalValueProp <- reactive({
      
      if(input$confLevelnProp == "90%") {
        critVal <- 1.645
      } else if(input$confLevelnProp == "95%") {
        critVal <- 1.96
      } else if(input$confLevelnProp == "99%") {
        critVal <- 2.576
      }
      return(critVal)
    })
    ...
    output$sampSizePropEstimate <- renderUI({
      ...
      if(isTRUE(input$normalDistribution)) {
        n <- getSampSizeEstProp(criticalValueProp(), input$sseTargetProp, input$ssePropMargErr, input$ssePropWoI)
        nEstimate <- ceiling(n)
